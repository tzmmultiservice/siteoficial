    ...
    <>
      ...
      <section className="pt-32 pb-16 relative">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <p className="text-primary-orange text-sm font-semibold tracking-widest uppercase mb-3">Catálogo completo</p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {PLACEHOLDER_SERVICES.map((service, i) => (
              <div
                key={service.name}
                className={`glass-card rounded-2xl p-7 flex gap-6 opacity-0 animate-fade-in-up stagger-${Math.min(i + 1, 8)}`}
              >
                <div className="w-14 h-14 rounded-xl gradient-brand flex items-center justify-center text-white shrink-0">
                  {ICON_MAP[service.icon] || ICON_MAP.settings}
                </div>
                <div className="flex-1">
                  <h3 className="font-bold text-white text-xl mb-2">{service.name}</h3>
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    ...
                    <Link href="/agendar" className="text-primary-orange hover:text-primary-gold font-semibold text-sm transition-colors">
                      Agendar →
                    </Link>
                  </div>
                </div>
      ...
      <section className="relative py-20 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 text-center">
          ...
          <Link
            href="/agendar"
            className="inline-flex items-center gap-2 bg-white/95 hover:bg-white text-gray-900 font-bold py-3 px-8 rounded-xl transition-all duration-300 hover:shadow-[0_10px_40px_rgba(0,0,0,0.3)]"
          >
            Agendar Agora
          </Link>
        </div>
